...
describe('Envelope - Fluxo de Integração', () => {
  ...
  describe('Validações de Negócio', () => {
    ...
    it('deve permitir apenas usuários autenticados', async () => {
      (getAuthenticatedUser as jest.Mock).mockRejectedValue(new Error('User not authenticated.'));

      await expect(create(mockFormData)).rejects.toThrow('User not authenticated.');
      await expect(get()).rejects.toThrow('User not authenticated.');
      await expect(update('id', mockFormData)).rejects.toThrow('User not authenticated.');
  ...
  describe('Segurança e Isolamento', () => {
    it('deve isolar envelopes por usuário', async () => {
      ...
      (getAuthenticatedUser as jest.Mock).mockResolvedValue({ user: otherUser });
  ...
  describe('Tratamento de Erros', () => {
    it('deve tratar erros de banco de dados graciosamente', async () => {
      ...
      mockPrisma.envelope.findFirst.mockRejectedValue(new Error('Database connection failed'));

      await expect(create(mockFormData)).rejects.toThrow('Database connection failed');

      // Simular erro de validação
      mockPrisma.envelope.findFirst.mockResolvedValue(null);
      mockPrisma.envelope.create.mockRejectedValue(new Error('Validation failed'));

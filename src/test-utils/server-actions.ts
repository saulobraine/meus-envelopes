...
export const setupEnvelopeActionsSuccess = () => {
  mockEnvelopeActions.create.mockResolvedValue(undefined as never);
  ...
  mockEnvelopeActions.update.mockResolvedValue(undefined as never);
  mockEnvelopeActions.remove.mockResolvedValue(undefined as never);
};

// Configurações de erro para envelope actions
export const setupEnvelopeActionsError = (errorMessage = "Erro de teste") => {
  mockEnvelopeActions.create.mockRejectedValue(
    new Error(errorMessage) as never
  );
  mockEnvelopeActions.get.mockRejectedValue(new Error(errorMessage) as never);
  mockEnvelopeActions.update.mockRejectedValue(
    new Error(errorMessage) as never
  );
  mockEnvelopeActions.remove.mockRejectedValue(
    new Error(errorMessage) as never
  );
};
...
export const setupTransactionActionsSuccess = () => {
  mockTransactionActions.create.mockResolvedValue(undefined as never);
  ...
  mockTransactionActions.update.mockResolvedValue(undefined as never);
  mockTransactionActions.remove.mockResolvedValue(undefined as never);
  mockTransactionActions.removeAll.mockResolvedValue(undefined as never);
};

// Configurações de erro para transaction actions
export const setupTransactionActionsError = (
  errorMessage = "Erro de transação"
) => {
  mockTransactionActions.create.mockRejectedValue(
    new Error(errorMessage) as never
  );
  mockTransactionActions.get.mockRejectedValue(
    new Error(errorMessage) as never
  );
  mockTransactionActions.update.mockRejectedValue(
    new Error(errorMessage) as never
  );
  mockTransactionActions.remove.mockRejectedValue(
    new Error(errorMessage) as never
  );
  mockTransactionActions.removeAll.mockRejectedValue(
    new Error(errorMessage) as never
  );
};
...
export const clearAllActionMocks = () => {
  Object.values(mockEnvelopeActions).forEach((mock) =>
    (mock as jest.Mock).mockClear()
  );
  Object.values(mockDashboardActions).forEach((mock) =>
    (mock as jest.Mock).mockClear()
  );
  Object.values(mockTransactionActions).forEach((mock) =>
    (mock as jest.Mock).mockClear()
  );
};
...
export const setupActionScenario = (
...
export default serverActionsUtils;
